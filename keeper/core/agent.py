...
"""
    ...
    def _handle_chat(self, entities: Dict[str, Any]) -> str:
        ...
        return """👋 你好！我是 Keeper，你的智能运维助手。以下是我能帮你做的事：

🖥️  **服务器巡检** — "检查本机" / "检查 192.168.1.100" / "服务器状态"
🔍  **批量巡检** — "批量巡检所有主机" / "检查所有机器"
🛡️  **漏洞扫描** — "扫描漏洞" / "扫描 192.168.1.100" / "全面扫描"
📊  **报告导出** — "导出为 JSON" / "生成 HTML 报告" / "保存为 Markdown"
📝  **日志查询** — "查看最近的操作记录" / "过去 24 小时做了什么" / "查看系统日志"
☸️  **K8s 管理** — "检查 K8s 集群状态" / "查看 Pod 的日志" / "重启 my-app deployment"
🐳  **Docker 管理** — "查看 Docker 容器状态" / "查看镜像占用" / "重启 xxx 容器"
🔧  **根因分析** — "分析一下为什么 CPU 高" / "帮我排查生产环境问题"
🌐  **网络诊断** — "测试 8.8.8.8 的延迟" / "检查 192.168.1.100 的 3306 端口通不通"
⏰  **定时任务** — "每 30 分钟检查一次" / "每天早上 9 点巡检"
🩹  **自动修复** — "帮我修复服务器问题" / "帮我清理一下磁盘"
🔒  **证书监控** — "检查 SSL 证书" / "看看证书有没有过期"
📢  **飞书通知** — "发送到飞书" / "推送巡检结果"
💻  **软件安装** — "安装 nmap" / "在 192.168.1.100 上安装 xxx"
🔎  **问题排查** — "有没有什么问题" / "系统健康吗"
⚙️  **配置管理** — "把 CPU 阈值设为 80%" / "切换到 production 环境"

你可以直接说"帮助"随时查看此列表。有什么需要帮忙的吗？"""
