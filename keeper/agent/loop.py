...
**你的核心能力：**
你有 23 个运维工具可用，包括服务器巡检、K8s 管理、Docker 管理、网络诊断、漏洞扫描、SSL 证书检查、系统日志查询、进程管理、Runbook 标准化运维（磁盘清理/服务重启/日志轮转）等。
...
## 效率约束（重要）
- **最多调用 2 轮工具**：第 1 轮收集数据，第 2 轮补充（仅在必要时），然后必须给出最终回答
- **不要重复调用同一工具**：如果已经获取了数据，不要再调同一个工具
- **拿到数据后立即分析**：工具返回结果后，你应该直接分析并给出结论，不要继续调工具
- **对于简单查询（如检查配置、查看状态）**：1 轮工具调用足够，拿到结果直接回答
...
class AgentLoop:
    ...
    def _make_wrapped_tool(self, original_tool, tool_name, safety_level):
        ...
        工具结果末尾追加 [最终回答指令]，强制 LLM 在看到工具结果后
        给出最终回答，而不是继续调用工具（减少不必要的 LLM API 调用）。
        """
        ...
        def wrapped_func(**kwargs):
            # 检查是否重复调用同一工具（防止 LLM 循环）
            if (agent_loop._last_tool_called == tool_name
                    and agent_loop._consecutive_tool_calls >= 1):
                return (
                    f"[警告] 你已连续调用 {tool_name}，"
                    f"请根据已有结果给出结论，不要重复调用同一工具。"
                )
            ...
            # 在工具结果末尾追加"最终回答指令"，强制 LLM 给最终回答
            # 这能显著减少不必要的后续 LLM 调用
            if isinstance(result, str):
                result = result.rstrip() + (
                    "\n\n[最终回答指令] 以上是你请求的数据。"
                    "请直接根据这些数据给出最终回答，不要再调用任何工具。"
                    "如果数据已经足够，请停止工具调用并给出结论。"
                )
    ...
    def _run_langgraph(
        ...
        try:
                    ...
                    for msg in items:
                        ...
                        if tool_calls:
                            for tc in tool_calls:
                                ...
                                if tname == self._last_tool_called:
                                    self._consecutive_tool_calls += 1
                                else:
                                    self._consecutive_tool_calls = 0
                                self._last_tool_called = tname

                                if self._consecutive_tool_calls >= 2:
                                    _emit(callback, {
                                        "type": "warning",
                                        "message": f"{tname} 连续调用 {self._consecutive_tool_calls + 1} 次，LLM 正在循环。",
                                    })
            ...
            delay = 5 if is_rate_limit else 0
            _emit(callback, {
                "type": "warning",
                "message": f"流式异常: {type(e).__name__}，{f'等待 {delay}s 后退避 ' if is_rate_limit else ''}降级到阻塞模式",
            })
            if delay:
                time.sleep(delay)
            try:
                ...
                # 阻塞模式也失败了，尝试带退避的最后一次重试
                if is_rate_limit:
                    retry_delay = 15
                    _emit(callback, {
                        "type": "warning",
                        "message": f"阻塞模式仍触发 {type(e2).__name__}，等待 {retry_delay}s 后重试",
                    })
                    ...
                    try:
                        ...
                        _emit(callback, {
                            "type": "warning",
                            "message": f"LLM API 持续限流，返回已收集的数据",
                        })
                        ...
                        if all_raw_messages:
                            ...
                            if response:
                                ...
                                return response
                        # 返回工具结果摘要
                        response = f"[LLM API 限流] 无法获取最终回复。已执行的工具:\n"
                        if turn.tool_calls:
        ...
        for loop_i in range(self.MAX_LOOPS):
            ...
            if not response.tool_calls:
                ...
                for tc in response.tool_calls:
                    ...
                    if self._consecutive_tool_calls >= 2:
                        result = f"[提示] 你已连续调用 {tool_name} {self._consecutive_tool_calls + 1}次，请尝试其他工具或给出结论。"
                        _emit(callback, {
                            "type": "warning",
                            "message": f"{tool_name} 连续调用 {self._consecutive_tool_calls + 1} 次，请尝试其他工具",
                        })
                    else:
                        _emit(callback, {
                            "type": "tool_call",
